...
```bat
.\run.bat debug abap_frontend lint path\to\zreport.abap
```

Human output is silent when no findings are emitted. When findings exist, diagnostics use the same
source-highlighted format as `abap_frontend analyze`.

Use JSON for CI or downstream tooling:

```bat
.\run.bat debug abap_frontend lint --json --pretty path\to\zreport.abap
.\run.bat debug abap_frontend lint --json --with-project path\to\zreport.abap
.\run.bat debug abap_frontend lint --json --all-files path\to\workspace
```

`--with-project` discovers the workspace around `FILE`, applies `[lints]` from `abapls.toml`, and
uses the same project-loading conventions as `abap_frontend analyze`. Without
