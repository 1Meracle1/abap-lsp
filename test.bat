...
"%ODIN_EXE%" check "%ROOT%src\workspace" -no-entry-point %ODIN_FLAGS% || exit /b
...
"%ODIN_EXE%" test "%ROOT%src\dependency_store" %ODIN_FLAGS% %TEST_FLAGS% || exit /b
"%ODIN_EXE%" test "%ROOT%src\workspace" %ODIN_FLAGS% %TEST_FLAGS%
