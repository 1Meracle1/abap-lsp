...
use abap_symbols::perf_api::{collect_unit_only, resolve_unit_only, validate_single_unit};

const DEFAULT_SAMPLE_PATH: &str = r"D:\dev\abap\prod_rep_check\perf-samples\CL_GUI_ALV_GRID.abap";
const PERF_SAMPLE_URI: &str = "file:///symbols_validate_perf_sample.abap";
...
fn run() -> Result<(), String> {
    ...
    let collect_resolve_start = Instant::now();
    let mut prepared = collect_unit_only(PERF_SAMPLE_URI, &source, &parsed);
    resolve_unit_only(&mut prepared);
    let collect_resolve_elapsed = collect_resolve_start.elapsed();

    let mut warmup_diagnostics = 0usize;
    let warmup_start = Instant::now();
    for _ in 0..config.warmup_iterations {
        let project = validate_single_unit(prepared.clone());
        warmup_diagnostics = project.diagnostics.len();
    }
    ...
    println!("collect_and_resolve_prep={collect_resolve_elapsed:?}");
    println!(
        "prepared_symbols={} prepared_references={} prepared_diagnostics={}",
    ...
    println!(
        "measure_iterations={} diagnostics_per_validate={} total_diagnostics={}",
        config.iterations, warmup_diagnostics, total_diagnostics
    );
    println!(
...
fn print_usage() {
    println!("Usage: cargo run -p abap_symbols --example build_symbols_validate_perf -- [options]");
    println!("Options:");
    println!("  --sample <path>       ABAP source file to analyze");
    println!("  --iterations <n>      Measured validate iterations (default: 20)");
