...
"#;
    ...
    assert!(unit.symbols.iter().any(|symbol| {
        symbol.kind == SymbolKind::Form && symbol.name.as_ref() == "run"
    }));
    assert!(unit.symbols.iter().any(|symbol| {
    ...
    assert!(unit.scopes.iter().any(|scope| scope.kind == ScopeKind::Form));
    assert!(unit.scopes.iter().any(|scope| scope.kind == ScopeKind::Method));
}
