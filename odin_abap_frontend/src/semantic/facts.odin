...
token_selector_access :: proc(
	...
	base := tokens[start]
	if !token_ident_like(base) {
		return {}, start, false
	}
	i := start + 1
	segments := make([dynamic]Field_Access_Segment, 0, 2, c.allocator)
	...
	for i + 1 < len(tokens) {
		...
		field := tokens[i + 1]
		if !token_ident_like(field) && !(op == "->" && field.text == "*") {
			break
		}
		if !seen && (op == "=>" || op == "~") {
			namespace = .Type
		...
		append(
			...
			Field_Access_Segment {
				name = canonical_name(field.text, c.allocator),
				range = field.range,
			},
		)
		seen = true
	...
	return Field_Access {
			...
			base_name = canonical_name(base.text, c.allocator),
...
token_expr_skip_keyword :: proc(text: string) -> bool {
	return(
		ascii_equal_ignore_case(text, "AND") ||
		ascii_equal_ignore_case(text, "OR") ||
		ascii_equal_ignore_case(text, "NOT") ||
		ascii_equal_ignore_case(text, "IS") ||
		ascii_equal_ignore_case(text, "IN") ||
		ascii_equal_ignore_case(text, "LET") ||
		ascii_equal_ignore_case(text, "FOR") ||
		ascii_equal_ignore_case(text, "WHERE") ||
		ascii_equal_ignore_case(text, "UNTIL") ||
		ascii_equal_ignore_case(text, "WHILE") ||
		ascii_equal_ignore_case(text, "INIT") ||
		ascii_equal_ignore_case(text, "NEXT") ||
		ascii_equal_ignore_case(text, "WHEN") ||
		ascii_equal_ignore_case(text, "THEN") ||
		ascii_equal_ignore_case(text, "ELSE") ||
		ascii_equal_ignore_case(text, "TYPE") ||
		ascii_equal_ignore_case(text, "LIKE") ||
		ascii_equal_ignore_case(text, "VALUE") ||
		ascii_equal_ignore_case(text, "DATA") ||
		ascii_equal_ignore_case(text, "FIELD") ||
		ascii_equal_ignore_case(text, "SYMBOL") \
	)
}
