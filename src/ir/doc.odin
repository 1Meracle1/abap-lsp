...
ABAP source and later bytecode, runtime, analysis, or debug consumers.
...
- deterministic printing, read-only walking/querying, verification, and the
  first bytecode lowering boundary.
...
2. Lower checked ABAP into a `Module`.
   ...
   expression facts, type facts, SQL facts, and call facts. It records borrowed
   semantic and AST/source provenance on types, slots, functions, operations,
   and diagnostics where that provenance is available.

3. Verify before relying on IR.

   Use `verify_module(&result.module, allocator)` before handing IR to bytecode,
   runtime, analysis, or other consumers that assume well-formed control,
   value, and effect flow. `Verify_Result` reports invalid ids, missing
...
5. Cross into bytecode only at the bytecode boundary.

   The `src:ir/bytecode` package verifies IR modules, rejects unsupported
   operations with their recorded source and message, and lowers structural core
   operations to native bytecode through `lower_module`. Modeled ABAP, call,
   table, SQL, and system-field operations lower to callback-backed bytecode records for a
   future runtime consumer. It assigns one bytecode register per IR value,
   records block offsets in IR block order, and preserves branch arguments as
   edge metadata. Bytecode/runtime-specific execution, persistence, and external
   I/O should not leak back into general IR construction or lowering.
...
- terminator helpers close blocks with branches, conditional branches, or
  returns.
...
- `Module` owns its dynamic arrays and strings allocated through its allocator.
  ...
  `Lower_Result`.
- `Verify_Result` and bytecode results own their diagnostics/output arrays and
  must be destroyed with their matching destroy procedures.
...
- `Source_Loc.file`, `Source_Loc.node`, `Function.entity`, `Slot.entity`, and
  `Type.semantic_type` are borrowed pointers into the AST/semantic snapshot used
  to build the module. They are provenance, not persistent ids. Consumers that
  read them must keep the parsed/semantic snapshot alive.
...
if bytecode or runtime support has not caught up yet.
...
- Semantic facts should be consumed from semantic-owned APIs or records. IR may
  ...
  policy.
- Successful bytecode lowering must produce only instructions the runtime can
  consume directly. ABAP/domain behavior that needs host state, I/O, SQL,
  internal-table semantics, system fields, or dynamic dispatch must be modeled
  as runtime callbacks before it is admitted into bytecode.
*/
