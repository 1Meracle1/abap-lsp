...
@(test)
typepool_symbol_lookup_returns_unique_source_artifacts :: proc(t: ^testing.T) {
	path := workspace_store_path("candidate_lookup_typepool_symbol.sqlite3")
	store, err := dependency_store_from_override_path(path, context.allocator)
	testing.expect_value(t, err, Store_Error.None)
	profile := sample_profile()
	symbols := [?]string{"GFW_BOOLEAN", "GFW_FALSE"}
	...
	records, lookup_err := find_typepool_artifacts_for_symbols(
		&store,
		&profile,
		[]string{"gfw_boolean", "gfw_false"},
		context.allocator,
	)
	testing.expect_value(t, lookup_err, Store_Error.None)
	testing.expect_value(t, len(records), 1)
	testing.expect_value(t, records[0].object_name, "gfw")
}
